...
def clamp_coordinates(x: int, y: int) -> tuple[int, int]:
    x = max(BORDER, min(x, WIDTH_DISPLAY - WIDTH_RECTANGLE - BORDER))
    y = max(BORDER, min(y, HEIGHT_DISPLAY - HEIGHT_RECTANGLE - BORDER))
    return x, y
...
while run:
    ...
    COORD_X, COORD_Y = clamp_coordinates(COORD_X, COORD_Y)
